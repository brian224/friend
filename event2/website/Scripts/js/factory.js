(function (window , document , jQuery , undefined) {
    ...
    factory.prototype.owlMediaInit = function(element) {
        ...
        for (var i = 0, $elem = jQuery(element); i < $elem.length; i++) {
            if ( $elem.eq(i).find('.owl-item.active > *').attr('data-media') ) {
    ...
    factory.prototype.mediaAppend = function(element , callback) {
        var _url        = element.attr('data-media'),
            ...
            _stopAt     = /stopat\=/.test(_url) ? ( ( /stopat\=([^?&#]*)/.exec(_url)[1] ) | 0 ) : 0;
                ...
                events: {
                    ...
                    onStateChange : function(event) {
                        ...
                        if ( event.data === 0 ) {
                            ...
                            $(story._mainCut).attr('data-choice', 'help');
                            event.target.destroy();
                            projects.$w.delay(100).queue(function(){
                        ...
                        if ( event.data === 5 ) {
                            event.target.destroy();
                            $('.jQ-video').empty();
                            projects._media._loaded = undefined;
                            delete window['YT'];
                            projects._media._player = [];
                        }
